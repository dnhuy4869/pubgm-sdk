...
TArray<struct FItemStateData> UAIStateInfoComponent::QueryItemStates(int MaxBoxNum, int MaxItemNum, float AirDropBoxRangeInner, float AirDropBoxRangeOuter, float DeathBoxRange, float PickUpWrapperRange, float FindBuildingRange, bool InIsUseItemSpotLoc)
{
	static UFunction *pFunc = 0;
	if (!pFunc)
...
TArray<struct FAIStateInfo> UAIStateInfoComponent::GetAllPlayerStateInfo()
{
	static UFunction *pFunc = 0;
	if (!pFunc)
...
// (Final, Native, Public)
// Parameters:
// class ABaseAIController*       InController                   (ConstParm, Parm, ZeroConstructor, IsPlainOldData)
// TArray<struct FAIBackpackItem> ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm)

TArray<struct FAIBackpackItem> UAIStateInfoComponent::GetAIPlayerBackpackItems(class ABaseAIController* InController)
{
	static UFunction *pFunc = 0;
	if (!pFunc)
...
TArray<struct FAINearbyThrown> UAIStateInfoComponent::GetAINearbyThrownState(class ASTExtraBaseCharacter* InCharacter, float InRange, float InCheckAngle, int MaxNum)
{
	static UFunction *pFunc = 0;
	if (!pFunc)
